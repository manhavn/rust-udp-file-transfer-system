#!/bin/bash
# Script build server trên nhiều môi trường OS và Architecture
...
echo -e "${BLUE}=== UDP Transfer System - Build Script cho Server ===${NC}"
...
echo "3) Linux aarch64 (ARM64 - Raspberry Pi, Server ARM)"
...
echo -e "${BLUE}[2/2] Đang biên dịch server ở chế độ Release...${NC}"
if [ -z "$TARGET" ]; then
    $BUILD_CMD --release --bin server
    BUILD_STATUS=$?
    ...
    if [[ "$TARGET" == *"windows-gnu"* ]]; then
        ...
        RUSTFLAGS="-C link-args=-static" $BUILD_CMD --release --target $TARGET --bin server
    else
        $BUILD_CMD --release --target $TARGET --bin server
    fi
    BUILD_STATUS=$?
