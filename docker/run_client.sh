...
if [ -z "$1" ] || [ "$1" == "--help" ] || [ "$1" == "-h" ]; then
    ...
    echo "  SERVER_IP          Địa chỉ IP của Server (mặc định: host.docker.internal)"
...
DOCKER_SERVER_IP=${SERVER_IP:-"host.docker.internal"}

# Mount thư mục chứa file và thực thi hoàn toàn bằng biến môi trường
docker run --rm -it \
  --add-host=host.docker.internal:host-gateway \
  -e FILE_PATH="/data/$FILE_NAME" \
  -e SERVER_IP="$DOCKER_SERVER_IP" \
